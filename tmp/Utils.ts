...
export function createEventScheduleStartingOnADay(uniquePrefix: string, startDay: number) {
    ...
    let eventSchedule = EventSchedule.create({
        eventStart: day,
...
export function addAPeriodWithEventOnDayAndEventUsingStudyStart(
    ...
    event1Day: number,
    event2Name: string,
    event2DaysAfterStudyStart: number,
): StudyConfiguration {
...
export function addEventScheduledOffCompletedEvent(
    ...
    const timeAmount = TimeAmount.create({
        value: event2DaysAfterEvent1,
        unit: timeUnit,
    });
