# Timeline Table
| Visit Name | Alternative Name | Phase | Window (-) | Day/Date | Window (+) |
| :---------- | :---------- | :---------- | :---------- | :---------- | :---------- |
| V1 |   | Screening | 0 | 1 | 0 |

---

# Screening


## V1

<p>This is visit V1</p>

This event is first scheduled  as the start day of the study with a window of with a window of 0 day(s) before and 0 day(s) after and no compliance window


### Task: Task 1

<p>The description of the first and only checklist for V1...</p>


#### Step 1: Step

**REFERENCES**

- undefined http://google.com

**PEOPLE**

- Mike Vogel ([object Object]) mike.vogel@triradial.com 908-399-7199
