<script lang="ts">
    ...
    import { SystemAccess, SystemAccessReference, type StudyConfiguration } from "@freon4dsl/study-configuration";
    ...
    // Share handler - converts SystemAccess to shared SystemAccess and replaces with SystemAccessReference
    const handleShare = (event: MouseEvent | KeyboardEvent) => {
        event.stopPropagation();
        event.preventDefault();

        const node = box.node;
        if (node.freLanguageConcept() === "SystemAccess") {
...
{#if inlineDisplay}
    ...
    >
        ...
        <SelectableWrapperComponent {box} {editor} onDelete={canDelete ? handleDelete : undefined}>
            <span class="selectable-list-item-content-inline">
...
<style>
    ...
    .selectable-list-item-content-inline {
        display: inline-flex;
        align-items: center;
        gap: 0.25rem;
    }
