<script lang="ts">
    ...
    import { StudyConfiguration, studyTimelineChart } from "@freon4dsl/study-configuration";
    ...
    let { patientId, studyId, selectedDate } = $props<{ patientId: string; studyId: string; selectedDate: Date }>();
    ...
    export function refresh() {
        dispatch("refresh");
        buildChart(studyId, selectedDate);
    }
    ...
    $effect(() => {
        if (selectedDate) {
            ...
            setDrawerTitle("patientStudyTimelineChart", `Study Timeline Chart - ${dateStr}`);
        }
    });

    $effect(() => {
        console.log("[PatientStudyTimelineChartDrawer] $effect studyId:", studyId, "selectedDate:", selectedDate);
        if (studyId && selectedDate) {
            buildChart(studyId, selectedDate);
        }
    });

    async function buildChart(id: string, date: Date) {
        console.log("build PatientStudyTimelineChart: ", id, date);
        isLoading = true;
        showChart = false;
        error = null;
        try {
            ...
            // Get the timeline chart for the selected date
            const rtObject = studyTimelineChart(unit, normalizedDate, false, true) as RtString;
