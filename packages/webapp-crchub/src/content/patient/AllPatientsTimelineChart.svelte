<script lang="ts">
     ...
     */
    async function getPatientAndStudyUnits(studyId: string): Promise<{ patientInfo: any; studyConfig: StudyConfiguration }> {
        const modelManager = ModelManager.getInstance();
        
        const patientInfo = await modelManager.getModelUnitWithoutOpening(studyId, "PatientInfo") as any;
        if (!patientInfo) {
