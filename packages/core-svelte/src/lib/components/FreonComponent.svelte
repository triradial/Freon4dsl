<script lang="ts">
    ...
    import { contextMenu, contextMenuVisible, selectedBoxes, viewport, componentId, shouldBeHandledByBrowser } from "$lib/components/svelte-utils/index.js";
    ...
    const refreshRootBox = (why?: string) => {
        ...
        LOGGER.log(
            "REFRESH " +
                ...
                rootBox?.id +
                " unit " +
                (!!rootBox?.node ? rootBox.node["name"] : "undefined"),
        );
    };
...
<div class={"freon-component"} on:keydown={onKeyDown} on:scroll={onScroll} bind:this={element} {id} role="group">
...
<ContextMenu bind:this={$contextMenu} items={[]} {editor} />
