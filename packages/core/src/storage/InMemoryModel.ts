...
export class InMemoryModel {
  ...
  constructor(
    languageEnvironment: FreEnvironment,
  ...
  async createUnit(
    name: string,
    unitConcept: string,
  ): Promise<FreModelUnit | InMemoryError> {
    ...
    if (notNullOrUndefined(newUnit)) {
      ...
      const response = await this.server.createModelUnit(
        this.model.name,
        newUnit,
      );
      if (response.errors.length > 0) {
  ...
  async deleteUnit(unit: FreModelUnit): Promise<void | InMemoryError> {
    const response = await this.server.deleteModelUnit(this.model.name, {
      name: unit.name,
      id: unit.freId(),
  ...
  async deleteUnitById(
    unitId: FreUnitIdentifier,
  ): Promise<void | InMemoryError> {
  ...
  async renameUnit(
    ...
    if (oldName === newName) {
      LOGGER.log(
        `renameUnit skipped: oldName and newName are the same (${oldName})`,
      );
      return;
    }
    LOGGER.log(`renameUnit from ${oldName} to ${newName}`);
    const response = await this.server.renameModelUnit(
      this.model.name,
      oldName,
      newName,
      unit,
  ...
  primChanged = (delta: FrePrimDelta) => {
    if (this.getUnits().includes(delta.unit)) {
      ...
      if (
        delta.owner.freIsUnit() &&
        delta.propertyName === "name" &&
        typeof (delta.oldValue === "string")
      ) {
        // Unit name changed !
        this.renameUnit(
          delta.oldValue as string,
          delta.newValue as string,
          delta.unit,
        );
        this.currentModelChanged();
      }
