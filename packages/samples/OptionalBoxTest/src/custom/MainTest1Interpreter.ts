...
import { Test1InterpreterInit } from "../freon/interpreter/Test1InterpreterInit.js";
 ...
 */
export class MainTest1Interpreter implements FreInterpreter {
    private static main: IMainInterpreter | null = null;

    constructor() {
        MainTest1Interpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(Test1InterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainTest1Interpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainTest1Interpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainTest1Interpreter.getMain().reset();
        try {
            return MainTest1Interpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
