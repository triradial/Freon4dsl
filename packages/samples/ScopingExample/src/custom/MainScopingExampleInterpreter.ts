...
import { ScopingExampleInterpreterInit } from "../freon/interpreter/ScopingExampleInterpreterInit.js";
 ...
 */
export class MainScopingExampleInterpreter implements FreInterpreter {
    private static main: IMainInterpreter | null = null;

    constructor() {
        MainScopingExampleInterpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(ScopingExampleInterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainScopingExampleInterpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainScopingExampleInterpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainScopingExampleInterpreter.getMain().reset();
        try {
            return MainScopingExampleInterpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
