...
import { ExternalModelInterpreterInit } from "../freon/interpreter/ExternalModelInterpreterInit.js";
...
export class MainExternalModelInterpreter implements FreInterpreter {
    ...
    constructor() {
        MainExternalModelInterpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(ExternalModelInterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainExternalModelInterpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainExternalModelInterpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainExternalModelInterpreter.getMain().reset();
        try {
            return MainExternalModelInterpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
