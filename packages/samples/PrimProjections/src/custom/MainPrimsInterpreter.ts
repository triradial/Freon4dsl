...
import { PrimsInterpreterInit } from "../freon/interpreter/PrimsInterpreterInit.js";
 ...
 */
export class MainPrimsInterpreter implements FreInterpreter {
    private static main: IMainInterpreter | null = null;

    constructor() {
        MainPrimsInterpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(PrimsInterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainPrimsInterpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainPrimsInterpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainPrimsInterpreter.getMain().reset();
        try {
            return MainPrimsInterpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
