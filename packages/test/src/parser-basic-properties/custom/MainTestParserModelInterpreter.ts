...
import { TestParserModelInterpreterInit } from "../freon/interpreter/TestParserModelInterpreterInit.js";
 ...
 */
export class MainTestParserModelInterpreter implements FreInterpreter {
    private static main: IMainInterpreter | null = null;

    constructor() {
        MainTestParserModelInterpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(TestParserModelInterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainTestParserModelInterpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainTestParserModelInterpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainTestParserModelInterpreter.getMain().reset();
        try {
            return MainTestParserModelInterpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
