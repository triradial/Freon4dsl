...
import { StudyConfigurationModelGrammarStr } from "./StudyConfigurationModelGrammar.js";
import { StudyConfigurationModelSyntaxAnalyser } from "./StudyConfigurationModelSyntaxAnalyser.js";
import { StudyConfigurationModelSemanticAnalyser } from "./StudyConfigurationModelSemanticAnalyser.js";
import {
    Agl,
    type LanguageIssue,
...
/**
 *   Class StudyConfigurationModelModelUnitReader is a wrapper that is able to parse all types of
 *   model units in the language.
 */
export class StudyConfigurationModelModelUnitReader implements FreReader {
    analyser!: StudyConfigurationModelSyntaxAnalyser;
    parser: LanguageProcessor<StudyConfigurationModel, MyContext> | null | undefined;
    isInitialized: boolean = false;

    initialize() {
        this.analyser = new StudyConfigurationModelSyntaxAnalyser();
        const res: LanguageProcessorResult<any, any> = Agl.getInstance().processorFromString(
            StudyConfigurationModelGrammarStr,
     ...
     */
    readFromString(sentence: string, metatype: string, model: StudyConfigurationModel, sourceName?: string): FreModelUnit {
        ...
        if (metatype === "Availability") {
            startRule = "Availability";
        } else if (metatype === "PatientInfo") {
            startRule = "PatientInfo";
        } else if (metatype === "PatientHistoryUnit") {
            startRule = "PatientHistoryUnit";
        } else if (metatype === "StudyConfiguration") {
            startRule = "StudyConfiguration";
        }

        // parse the input
        let unit: FreModelUnit | null = null;
        if (this.parser) {
            let parseResult: ProcessResult<StudyConfigurationModel> | undefined;
                ...
                if (notNullOrUndefined(model)) {
                    try {
                        ...
                        if (model.getUnits().some((existing) => existing.name === name)) {
                            ...
                            AST.change(() => {
                                model.addUnit(u);
                                const semAnalyser = new StudyConfigurationModelSemanticAnalyser();
