...
import { Previous, EventState } from "../language/index.js";

import { LanguageEnvironment } from "../index.js";

/**
 * This class implements the box provider for a single node of type Previous.
...
export class PreviousBoxProvider extends FreBoxProvider {
    ...
    private getDefault(): Box {
        return BoxFactory.horizontalLayout(
            this._node as Previous,
            "Previous-hlist-line-0",
            "",
            [
                BoxUtil.labelBox(this._node as Previous, "when the previous event is", "top-1-line-0-item-0"),
                BoxUtil.refReplacerBox(this._node as Previous, "eventState", "CustomSelectComponent", {
                    params: [{ key: "placeholder", value: "state" }],
                }),
                ,
                BoxUtil.partReplacerBox(this._node as Previous, "timeAmountPart", "CustomActionsComponent", {
    ...
    private getParser(): Box {
        return BoxFactory.horizontalLayout(
            ...
            "",
            [
                BoxUtil.labelBox(this._node as Previous, "when the previous event", "top-1-line-0-item-0"),
                BoxUtil.limitedBox(
                    this._node as Previous,
                    "eventState",
                    (selected: string | EventState) => {
                        (this._node as Previous).eventState = FreNodeReference.create<EventState>(selected, "EventState");
                    },
                    LimitedDisplay.SELECT,
                    LanguageEnvironment.getInstance().scoper,
                ),
                BoxUtil.getBoxOrAction(this._node as Previous, "timeAmountPart", "TimeAmountPart", this.mainHandler),
            ],
            { selectable: false },
        );
    }
