<script lang="ts">
    ...
    import Drawer, { AppContent, Header, Title } from "@smui/drawer";
    import { Content } from "@smui/card";

    import { mdiGithub, mdiWeb, mdiWeatherNight, mdiWeatherSunny, mdiHelp, mdiChevronRight, mdiChevronLeft, mdiRunFast, mdiUndo, mdiRedo } from "@mdi/js";
    ...
    let lightTheme: boolean = typeof window === "undefined" || window.matchMedia("(prefers-color-scheme: light)").matches;
    ...
    function switchTheme() {
        ...
        document.head.querySelector<HTMLLinkElement>('link[href$="/site-dark.css"]')?.insertAdjacentElement("afterend", themeLink);
...
<svelte:window on:beforeunload={onBeforeUnload} on:keydown={handleKeydown} />
...
<TopAppBar bind:this={topAppBar} variant="standard" dense>
    <Row>
        ...
        <Section align="end" toolbar>
            ...
            <IconButton aria-label="Undo" on:click={EditorRequestsHandler.getInstance().undo}>
                <Icon tag="svg" viewBox="0 0 24 24">
                    <path fill="currentColor" d={mdiUndo} />
                </Icon>
            </IconButton>
            <IconButton aria-label="Redo" on:click={EditorRequestsHandler.getInstance().redo}>
                <Icon tag="svg" viewBox="0 0 24 24">
...
<style>
    .space-right {
        /* used to put some space between the menu buttons */
        margin-right: 6px;
    }
    .drawer-space-right {
        /* used to make some space for the menus, otherwise an open menu falls behind the drawer */
        margin-right: 220px;
    }
    .main-frame {
        overflow: hidden;
