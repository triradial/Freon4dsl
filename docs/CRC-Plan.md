...
- Fix data saving and loading issues for study, patient, availability. Most of the existing studies in the data store are now empty folders. Need to determine what happened. Seems to have happened after my changes to saving a patient. 
- Is there an issue where we leave deleted studies as folders? At least clean-up stored studies to be only ones that work. **Graham & Mike**
...
- Close panels when switching to a different study / patient or update them to match the changed context
